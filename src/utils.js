...
export function storeApiKey(settings, provider, apikey, onError) {
  try {
    const schema = new Secret.Schema(settings, Secret.SchemaFlags.NONE, {
...
export function getApiKey(settings, provider, onComplete, onError) {
  try {
    ...
    if (onError) onError(`Failed to get apikey for ${provider}: ${e.message}`);
    else log(`Failed to get apikey for ${provider}: ${e.message}`);
  }
}
